...
with app.app_context():
    ...
    product = Product.query.filter_by(product_name='SHUNFA SF-737 – 3-Thread Overlock Machine').first()
    if product:
        product.product_name = 'SHUNFA SF-737 – 3-Thread Overlock Machine'
        db.session.commit()
        print('Product name updated!')
    else:
        product = Product(
            product_name='SHUNFA SF-737 – 3-Thread Overlock Machine',
            model_number='SF-737',
            description='Shunfa SF-737 is a 3-thread overlock machine used for edge finishing and light seaming in knitted and woven fabrics.',
    ...
    specs = [
        ('Brand', 'Shunfa'),
        ('Model', 'SF-737'),
        ('Type', '3-Thread Overlock Machine'),
        ('Threads', '3-thread'),
    ...
    correct_path = '/static/pictures/SHUNFA SF-737 – 3-Thread Overlock Machine.jpg'
    if not img:
