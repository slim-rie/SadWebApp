...
with app.app_context():
    ...
    product = Product.query.filter_by(product_name='SHUNFA SF-372 – Button Sew Machine').first()
    if product:
        # Update name to all caps SHUNFA for frontend filter
        product.product_name = 'SHUNFA SF-372 – Button Sew Machine'
        db.session.commit()
        print('Product name updated to all caps SHUNFA!')
    else:
        product = Product(
            product_name='SHUNFA SF-372 – Button Sew Machine',
            model_number='SF-372',
            description='Shunfa SF-372 is designed for button attachment on shirts, jackets, and other garments, providing secure stitching with adjustable settings.',
    ...
    correct_path = '/static/pictures/SHUNFA SF-372 Buttonsew machine.jpg'
    if not img:
